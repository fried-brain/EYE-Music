...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
       ...
       return client.sendTime(message.channel,"<:xmark:774976403514720267> | **Lavalink node not connected**");
        }
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setDescription(`<:check:774976288610975764> | Playlist Enqueued`);
                    //SongAddedEmbed.addField("Tracks in Queue", `\`${Searched.tracks.length}\` songs`, false);
                    ...
                    SongAddedEmbed.setAuthor(`Added to queue ▶`, client.user.displayAvatarURL());
                ...
                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**No matches found for - **" + SearchString);
                    ...
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`Playlist Enqueued ▶`, client.user.displayAvatarURL());
                    ...
                    if (!player.playing && !player.paused && !player.queue.size) player.play();
                    SongAddedEmbed.setAuthor(`Added ▶`, client.user.displayAvatarURL());
    ...
    SlashCommand: {
        ...
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
            if (!CheckNode || !CheckNode.connected) {
              return client.sendTime(interaction,"<:xmark:774976403514720267> | **Lavalink node not connected**");
            }
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    case "LOAD_FAILED":
                        ...
                        return client.sendError(interaction, `<:xmark:774976403514720267> | **There was an error while searching**`);

                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                        return client.sendTime(interaction, "<:xmark:774976403514720267> | **No results were found.**");
                    case "TRACK_LOADED":
                    ...
                    case "SEARCH_RESULT":
                        ...
                        let SongAdded = new MessageEmbed();
                            SongAdded.setAuthor(`Added to queue ▶`, client.user.displayAvatarURL());
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        Playlist.setAuthor(`Playlist added ▶`, client.user.displayAvatarURL());
                        Playlist.setDescription(`[${Searched.playlistInfo.name}](${interaction.data.options[0].value})`);
                        Playlist.addField("Tracks in queue", `\`${Searched.tracks.length}\` songs`, false);
                ...
                switch (res.loadType) {
                    ...
                    case "TRACK_LOADED":
                        ...
                        if (!player.playing && !player.paused && !player.queue.length) player.play();
                        let SongAddedEmbed = new MessageEmbed();
                            SongAddedEmbed.setAuthor(`Queued ▶`, client.user.displayAvatarURL());
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        await player.play();
                        let SongAdded = new MessageEmbed();
                        SongAdded.setAuthor(`Playlist added ▶`, client.user.displayAvatarURL());
                        ...
                        SongAdded.addField("Tracks in Queue", `\`${res.tracks.length}\` songs`, false);
                    ...
                    case "SEARCH_RESULT":
                        ...
                        if (!player.playing && !player.paused && !player.queue.length) {
                            ...
                            let SongAddedEmbed = new MessageEmbed();
                            SongAddedEmbed.setAuthor(`Track Added ▶`,client.user.displayAvatarURL())
                            SongAddedEmbed.setThumbnail(track.displayThumbnail());
                            SongAddedEmbed.setColor("RANDOM");
                            SongAddedEmbed.setDescription(`[${track.title}](${track.uri})`);
                            /*SongAddedEmbed.addField("Author", track.author, true);
