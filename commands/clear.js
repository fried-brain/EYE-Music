...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue || !player.queue.length || player.queue.length === 0)
      return client.sendTime(message.channel, "<:xmark:774976403514720267> | **Nothing is playing right now...**");
    ...
    player.queue.clear();
    await client.sendTime(message.channel, "<:check:774976288610975764> | **Cleared the queue!**");
  },
   ...
   */
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!player)
        return client.sendTime(interaction, "<:xmark:774976403514720267> | **Nothing is playing right now...**");

      if (!player.queue || !player.queue.length || player.queue.length === 0)
        return client.sendTime(interaction, "<:xmark:774976403514720267> | **Nothing is playing right now...**");
      player.queue.clear();
      await client.sendTime(interaction, "<:check:774976288610975764> | **Cleared the queue!**");
    },
  },
};
